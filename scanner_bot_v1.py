...
MIN_SCORE       = 5          # minimum score to consider a trade (paper mode = 5, live = 7)
...
def determine_direction(ind):
    ...
    PAPER MODE (v1):
    - ADX is optional (if it fails to calculate, default to 25)
    - Require 2+ of 4 signals aligned
    - Goal: Generate maximum data for validation
    - Let scoring and veto conditions filter weak setups
    """
    price   = ind.get("close", 0)
    ema20   = ind.get("ema20")
    ema50   = ind.get("ema50")
    ...
    rsi     = ind.get("rsi")
    adx     = ind.get("adx", 25)  # ← Default to 25 if None/missing
    
    # Must have core indicators
    if None in (price, ema20, ema50, plus_di, minus_di, rsi):
        return None
    
    # If ADX is None, use default of 25 (reasonable trend threshold)
    if adx is None:
        adx = 25
    
    # Both directions require minimum trend (ADX >= 20)
    if adx < 20:
        return None  # No trend = skip both directions
    ...
    # Require 2+ aligned signals (lowered from 3 to allow more entries)
    if long_signals >= 2:
        return "long"
    if short_signals >= 2:
        return "short"
...
def monitor_paper_positions(state, trades, prices):
    ...
    if state["daily_pnl"] <= -DAILY_LOSS_CAP and not state["cap_hit"]:
        state["cap_hit"] = True
        save_state(state)
...
def _print_banner(balance, open_count, cycle, state):
    ...
    print(f"Balance: ${balance:.2f}  │  Open: {open_count}/{MAX_POSITIONS}")
...
def _attempt_entries(state, trades, ranked_results, prices):
    if state.get("cap_hit"):
        return
    if state.get("balance_cache", 0) and state["balance_cache"] < ACCOUNT_FLOOR:
        return

    existing = set(state["paper_positions"].keys())
    slots = max(MAX_POSITIONS - len(existing), 0)
    if slots > 0:
    ...
    if len(state["paper_positions"]) >= MAX_POSITIONS and ranked_results:
...
def scan_loop():
    ...
    while True:
        ...
        if balance and balance < ACCOUNT_FLOOR:
            print(f"{Y}Account below floor (${ACCOUNT_FLOOR:.2f}). No new entries this cycle.{X}")
...
def main():
    ...
    print(f"  Loss cap  : ${DAILY_LOSS_CAP:.2f}")
    print(f"  Floor     : ${ACCOUNT_FLOOR:.2f}")
